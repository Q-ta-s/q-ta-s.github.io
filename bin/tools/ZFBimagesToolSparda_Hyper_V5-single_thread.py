...
class ZFBimagesTool(tk.Frame):
    ...
    def get_auto_image_size(self):
        img_size  = (int(self.img.default_width), int(self.img.default_height))
    ...
    def execute_create_zfb(self):
        ...
        self.process_zfb_files(arcade_mode=self.arcade_var.get())
    ...
    def process_zfb_files(self, arcade_mode=False):
        try:
            ...
            if self.img_mode.get() == "auto":
                auto_size = self.get_auto_image_size()
                self.img_hyperscreen_var.set(self.check_image_size(auto_size))
                img_w, img_h = auto_size
            ...
            if not input_folder or not output_folder or (not arcade_mode and (not core or not extension)) or not img_w or not img_h:
...
if __name__ == "__main__":
    ...
    root.title("ZFB Generator Hyper [ST]")
